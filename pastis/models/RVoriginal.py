...
def CCF_interpolate(RV, CCF, params):
    ...
    nstep = 100
    margin = 5
    depth = n.arange(nstep-2*margin+1,dtype=float)/nstep + float(margin)/nstep
    ...
    p[iRange,1] = n.array([(RV[i+1]-RV[i]-p[i,2]*(norm_CCF[i+1]**2-norm_CCF[i]**2))/(norm_CCF[i+1]-norm_CCF[i]) for i in iRange])
    p[iRange,0] = n.array([RV[i]-p[i,1]*norm_CCF[i]-p[i,2]*norm_CCF[i]**2 for i in iRange])

    # Indexes where "norm_CCF > dd"
    Indexes = n.array([[n.where(norm_CCF > dd)[0][0]-1,n.where(norm_CCF > dd)[0][-1]] for dd in depth])
    IndexBlue, IndexRed = [ind[0] for ind in Indexes],[ind[-1] for ind in Indexes]

    # Bisector definition
    bis_b = n.array([p[i_b,0]+p[i_b,1]*dd+p[i_b,2]*dd**2 for i_b,dd in zip(IndexBlue,depth)])
    bis_r = n.array([p[i_r,0]+p[i_r,1]*dd+p[i_r,2]*dd**2 for i_r,dd in zip(IndexRed,depth)])
...
def Vasy(RV, CCF, Gauss_params):
	...
	Vasy_res = n.sum((weight_r - weight_b) * average_weight) / n.sum(average_weight)
...
def asym_gaussian(p,x):
	...
	p : [c, rv0, sig, Asym]
	"""
	Gb = 1. - p[0]/100.*n.exp(-(x[n.where(x < p[1])[0]]-p[1])**2/(2.*(p[2]*(1.-p[3]))**2))
	Gr = 1. - p[0]/100.*n.exp(-(x[n.where(x >= p[1])[0]]-p[1])**2/(2.*(p[2]*(1.+p[3]))**2))
...
def get_FWHM(vsini, BmV, spectrograph, output = 'FWHM'):
	...
	if spectrograph == 'CORALIE' : 
		...
		A=1.9
		FWHM=n.sqrt((vsini**2.+A**2.*sigma0**2.)/A**2.)*(2.*n.sqrt(2.*n.log(2.)))
	...
	elif output == 'sigma' : return FWHM/(2.*n.sqrt(2.*n.log(2.)))
	else : return
...
def get_contrast(FWHM, BmV, z, spectrograph, mask, output = 'ctrs'):
	...
	if output == 'ctrs' : return 10**logW / n.sqrt(2.*n.pi) / (FWHM / (2.*n.log(2.))) * 100. # in % 
...
def profile_rot(x, x0, vsini, epsilon, ctrs, FWHM_instru = 9.9):
	...
	y1=1./n.sqrt(2.*n.pi*(FWHM_instru/sig)**2)*n.exp(-(x-x0)**2/(2.*(FWHM_instru/sig)**2))
...
def PASTIS_RV(t_rv, RVdatadict, *args):
    ...
    if len(args) == 1 and isinstance(args[0], PlanSys):
        ...
        if RVdatadict.has_key('RV')  :
            ...
            for p in args[0].planets:
                if p.orbital_parameters.spinorbit <> None :
                    if isinstance(p, Planet) :output_dict['RV'] += Pyarome.arome(p.get_true_lat(t_rv), p.ar*(1.-p.orbital_parameters.ecc**2)/(1.+p.orbital_parameters.ecc*n.sin(p.orbital_parameters.omega)), p.orbital_parameters.incl, p.orbital_parameters.spinorbit, [args[0].star.ua, args[0].star.ub], beta0=sigma0, Vsini=args[0].star.vsini, sigma0=CCF_width, zeta=args[0].star.zeta, Rp=p.Rp/p.star.R, units='degree')
                    elif isinstance(p, FitPlanet) :output_dict['RV'] += Pyarome.arome(p.get_true_lat(t_rv), p.ar*(1.-p.orbital_parameters.ecc**2)/(1.+p.orbital_parameters.ecc*n.sin(p.orbital_parameters.omega)), p.orbital_parameters.incl, p.orbital_parameters.spinorbit, [args[0].star.ua, args[0].star.ub], beta0=sigma0, Vsini=args[0].star.vsini, sigma0=CCF_width, zeta=args[0].star.zeta, Rp=p.kr, units='radian')
        ...
        # CCF parameters
        cond_RV_CCF = n.logical_and(RV_CCF_ALL > n.min(rv0) - 5.*max(FWHM)/2.3548, RV_CCF_ALL < n.max(rv0) + 5.*max(FWHM)/2.3548)
